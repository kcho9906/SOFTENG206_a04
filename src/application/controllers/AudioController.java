...
/**
 * This is a controller class for the scene "Audio.fxml" and is responsible for
 * everything relating to the Audio scene.
 */
public class AudioController implements Initializable {
	...
	@FXML
	private Label waitingFor;
	@FXML
	private Label currentKeywordLabel;
	@FXML
	private ProgressIndicator loadingCircle, loadingImagesCircle;
	@FXML
	...
	/**
	 * Method to search the wiki for the term in the input and displays the output
	...
	/**
	 * Method to preview how the selected text will sound when it has been saved.
	 * 
	 * @param event
	 */
	@FXML
	...
	void saveTextAction(ActionEvent event) {
		...
		if (validRange) {
			...
			String command = "espeak -v " + gender + " -s " + speed + " \"" + selectedText + "\" -w " + path + fileName
					+ "; lame -b 320 -h " + path + fileName + "; rm " + path + "*.wav";
	...
	void playAudioAction(ActionEvent event) {
		...
		String command = "ffplay -nodisp -autoexit src/audio/" + searchTerm + "/"
				+ audioListView.getSelectionModel().getSelectedItem();
	...
	/**
	 * Method to delete selected audio file. This will disable the next button if it
	 * is empty after deleting.
	 * 
	 * @param event
	 */
	@FXML
	...
	void returnToMenu(ActionEvent event) throws Exception {
		boolean returnToMenu = methodHelper.addConfirmationAlert("Return to Menu",
				"All progress with be lost. Are you sure?", "Continue", "Cancel");
		if (returnToMenu) {
			methodHelper.resetDirs();
	...
	private String mergeAudio() {
		...
		if (listForCreation.size() > 0) {
			...
			if (!bgMusic.equals("None")) {
				String bgMusicPath = System.getProperty("user.dir") + "/src/setupFiles/backgroundMusic/" + bgMusic
						+ ".mp3";
				command += "ffmpeg -y -i " + mergedPath + " -i " + bgMusicPath
						+ " -filter_complex amix=inputs=2:duration=shortest " + path + "output.mp3";
			}
		}
		return command;
	}
