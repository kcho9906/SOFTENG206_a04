...
public class MediaController implements Initializable {
	...
	@FXML
	private ImageView muteImage, playPauseImage;
	...
	private void setUpProperties() {
		...
		/**
		 * Listener to check if the volume slider has been changed and set the volume
		 * accordingly.
		 */
		...
		/**
		 * Listener to check if the time slider is adjusted and then change the time in
		 * the video to the part of the video.
		 */
		...
		_player.currentTimeProperty().addListener(new ChangeListener<Duration>() {
			...
			public void changed(ObservableValue<? extends Duration> observable, Duration oldValue, Duration newValue) {
				...
				String totalSecs = String.format("%02d",
						(int) (duration.toSeconds() - Integer.parseInt(totalMins) * 60));
				timeLabel.setText(time + "/" + totalMins + ":" + totalSecs);
				timeSlider.setValue(
						_player.getCurrentTime().toMinutes() / _player.getTotalDuration().toMinutes() * 100.00);
			}
		});
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		String command = "ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "
				+ _videoFile;
